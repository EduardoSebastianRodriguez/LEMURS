from navigationFunctions import realNavigation, generate_leader, generate_agents, learnNavigation, get_cmap
...
def main(seed=42, numAgentsTrain=4, numAgentsTests=4, numTests=20):
    ...
    learned_system.load_state_dict(torch.load('navigation'+str(numAgentsTrain)+'_learn_system_phsa.pth', map_location=device))
    with torch.no_grad():
        learned_system.eval()
    ...
    np.save('navigation_mean' + str(numAgentsTrain) + str(numAgentsTests) + '_loss_phsa.npy', loss_mean)
    np.save('navigation_std' + str(numAgentsTrain) + str(numAgentsTests) + '_loss_phsa.npy', loss_std)
