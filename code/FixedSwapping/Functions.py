...
class learnSystem(nn.Module):
    ...
    def navigation_dynamics(self, t, inputs):
        ...
        state[:, 0::6] = torch.kron(state_d[:, :, 0], torch.ones(self.na, 1, device=self.device))
        state[:, 1::6] = torch.kron(state_d[:, :, 1], torch.ones(self.na, 1, device=self.device))
        state[:, 2::6] = torch.kron(state_d[:, :, 2], torch.ones(self.na, 1, device=self.device))
        state[:, 3::6] = torch.kron(state_d[:, :, 3], torch.ones(self.na, 1, device=self.device))
        ...
        # H
        inputs_l = Variable(state.reshape(-1, self.na, 6).transpose(1, 2).data, requires_grad=True)
        H       = self.H.forward(inputs_l.to(torch.float32), L.reshape(-1, self.na, 6).transpose(1, 2))
    ...
    def navigation_dynamics_assess(self, inputs):
        ...
        # H
        inputs_l = Variable(state.reshape(-1, self.na, 6).transpose(1, 2).data, requires_grad=True)
        H = self.H._assess(inputs_l.to(torch.float32), L.reshape(-1, self.na, 6).transpose(1, 2))
        del state_d, state, L, pos, pos1, pos2, vel, vel1, vel2, LL
        Hgrad = torch.autograd.grad(H.sum(), inputs_l, only_inputs=True, create_graph=True)
        dH = Hgrad[0]
        dHq = dH[:, :2, :].reshape(-1, self.na, 2, self.na).transpose(2, 3)
...
class learnSystemMLP(nn.Module):
    ...
    def navigation_dynamics(self, t, inputs):
        ...
        # H
        inputs_l = Variable(state.data, requires_grad=True)
...
class learnSystemGNN(nn.Module):
    ...
    def navigation_dynamics(self, t, inputs):
        ...
        del L, inputs_d, inputs_l, inputs, pos, pos1, pos2, vel, vel1, vel2, state_d
