...
def main(numTrain=20000, numTests=20000, numSamples=5, seed_data=42, seed_train=42, numAgents=4):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'FS' + str(numAgents) + '_' + str(seed_train) + '_learn_system_phmlp.pth')
    ...
    # Save
    torch.save(TrainLosses, 'FS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phmlp_TrainLosses.pth')
    torch.save(TestsLosses, 'FS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phmlp_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MLP training for the fixed swapping problem')
