...
def main(numTrain=1000, numTests=200, numSamples=5, seed=42, numAgents=4):
    ...
    parameters = {"na": na, "d": d, "e": e, "a": a, "b": b, "c": c, "c1": c1, "c2": c2}
    ...
    # Store data
    torch.save(inputs_train, 'FS'+str(numAgents)+'_inputsTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(target_train, 'FS'+str(numAgents)+'_targetTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(inputs_tests, 'FS'+str(numAgents)+'_inputsTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(target_tests, 'FS'+str(numAgents)+'_targetTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Dataset generator for the fixed swapping problem')
