import torch
import argparse
from Functions import learnSystem, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed_data=42, seed_train=42, numAgents=4):
    ...
    learn_system = learnSystem(parameters)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'TVS' + str(numAgents) + '_' + str(seed_train) + '_learn_system_LEMURS.pth')
    ...
    # Save
    torch.save(TrainLosses, 'TVS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_LEMURS_TrainLosses.pth')
    torch.save(TestsLosses, 'TVS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_LEMURS_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LEMURS training for the time-varying swapping problem')
