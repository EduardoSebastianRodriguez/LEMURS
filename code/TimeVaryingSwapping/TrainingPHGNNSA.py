import torch
import argparse
from Functions import learnSystemGNNSA, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed_data=42, seed_train=42, numAgents=4):
    ...
    learn_system = learnSystemGNNSA(parameters)
    ...
    # Build dataset
    inputs   = torch.load('TVS'+str(numAgents)+'_inputsTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed_data)+'_.pth').to(device)
    target_2 = torch.load('TVS'+str(numAgents)+'_targetTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed_data)+'_.pth').to(device)

    # Build evaluation
    inputs_eval   = torch.load('TVS'+str(numAgents)+'_inputsTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed_data)+'_.pth').to(device)
    target_eval_2 = torch.load('TVS'+str(numAgents)+'_targetTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed_data)+'_.pth').to(device)
    current       = 100000000.0
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs-1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'TVS' + str(numAgents) + '_' + str(seed_train) + '_learn_system_phgnnsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'TVS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phgnnsa_TrainLosses.pth')
    torch.save(TestsLosses, 'TVS'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phgnnsa_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GNNSA training for the time-varying swapping problem')
