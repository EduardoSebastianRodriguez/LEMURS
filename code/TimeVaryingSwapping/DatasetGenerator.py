...
def main(numTrain=1000, numTests=200, numSamples=5, seed=42, numAgents=4):
    ...
    parameters = {"na": na, "d": d, "r": r, "e": e, "a": a, "b": b, "c": c, "c1": c1, "c2": c2}
    ...
    # Store data
    torch.save(inputs_train, 'TVS'+str(numAgents)+'_inputsTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(target_train, 'TVS'+str(numAgents)+'_targetTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(inputs_tests, 'TVS'+str(numAgents)+'_inputsTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(target_tests, 'TVS'+str(numAgents)+'_targetTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Dataset generator for the time-varying swapping problem')
