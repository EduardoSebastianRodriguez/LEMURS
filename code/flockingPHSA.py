import torch
import argparse
from flockingFunctions import learnFlocking, L2_loss, LD_loss, LH_loss, LV_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    learn_system = learnFlocking(parameters)
    ...
    # Build dataset
    inputs = torch.load('flocking'+str(numAgents)+'_inputsTrain_' + str(numTrain) + str(numTests) + str(numSamples) + str(42) + '_.pth').to(device)
    target_2 = torch.load('flocking'+str(numAgents)+'_target2Train_' + str(numTrain) + str(numTests) + str(numSamples) + str(42) + '_.pth').to(device)

    # Build evaluation
    inputs_eval = torch.load('flocking'+str(numAgents)+'_inputsTests_' + str(numTrain) + str(numTests) + str(numSamples) + str(42) + '_.pth').to(device)
    target_eval_2 = torch.load('flocking'+str(numAgents)+'_target2Tests_' + str(numTrain) + str(numTests) + str(numSamples) + str(42) + '_.pth').to(device)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            loss = alpha2 * loss2 #+ alphaH * lossH #+ alphaV * lossV #+ alphaH * lossH + alphaV * lossV #+ alphaD * lossD
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'flocking'+str(numAgents)+'_learn_system_phsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'flocking'+str(numAgents)+'_learn_system_phsa_TrainLosses.pth')
    torch.save(TestsLosses, 'flocking'+str(numAgents)+'_learn_system_phsa_TestsLosses.pth')
