import torch
import argparse
from navigationTvFunctions import learnNavigation, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    learn_system = learnNavigation(parameters)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            # Compute statistics
            print('Test results: Loss = %.6f' % loss.detach().cpu().numpy())
            print('-------------------------------------------------')

            # Store model
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'navigationTv'+str(numAgents)+'_learn_system_phsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'navigationTv'+str(numAgents)+'_learn_system_phsa_TrainLosses.pth')
    torch.save(TestsLosses, 'navigationTv'+str(numAgents)+'_learn_system_phsa_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='navigation time-varying')
