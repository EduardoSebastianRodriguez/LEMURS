...
from flockingFunctions import realFlocking, generate_leader, generate_agents
...
def main(numTrain=1000, numTests=200, numSamples=5, seed=42, numAgents=4):
    ...
    for k in range(num_traj_train):
        ...
        print('Flocking training instance '+str(k)+'.')
        # if k>=0:
        #     laplacian = real_system.laplacian(real_system.ha, trajectory[-1, : 2 * na])
        #     fig, ax = plt.subplots()
        #     for i in range(na):
        #         for j in range(i+1, na):
        #             if laplacian[i, j] != 0:
        #                 ax.plot([trajectory[-1, 2*i], trajectory[-1, 2*j]], [trajectory[-1, 2*i+1], trajectory[-1, 2*j+1]], 'b', linewidth=3)
        #     for i in range(na):
        #         ax.plot(trajectory[:, 2*i], trajectory[:, 2*i+1], 'k')
        #         ax.plot(trajectory[0, 2*i], trajectory[0, 2*i+1], 'go', markersize=14)
        #         ax.plot(trajectory[-1, 2*i], trajectory[-1, 2*i+1], 'bs', markersize=14)
        #     ax.plot(trajectory[0, 4*na], trajectory[0, 4*na + 1], 'rx', markersize=24)
        #     plt.xlabel('x $[$m$]$')
        #     plt.ylabel('y $[$m$]$')
        #     plt.show()
    ...
    for k in range(num_traj_tests):
        ...
        print('Flocking test instance '+str(k)+'.')
        # if k==0:
        #     laplacian = real_system.laplacian(real_system.ha, trajectory[-1, : 2 * na])
        #     fig, ax = plt.subplots()
        #     for i in range(na):
    ...
    # Store data
    torch.save(inputsTrain,   'flocking'+str(numAgents)+'_inputsTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(targetTrain_2, 'flocking'+str(numAgents)+'_target2Train_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(inputsTests,   'flocking'+str(numAgents)+'_inputsTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
    torch.save(targetTests_2, 'flocking'+str(numAgents)+'_target2Tests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth')
