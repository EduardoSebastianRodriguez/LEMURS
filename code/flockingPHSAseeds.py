...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phsa_TrainLosses.pth')
    torch.save(TestsLosses, 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phsa_TestsLosses.pth')
