import torch
import argparse
from navigationTvFunctions import learnNavigationGNN, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs-1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system, 'navigationTv'+str(numAgents)+'_learn_system_phgnn.pth')
    ...
    # Save
    torch.save(TrainLosses, 'navigationTv'+str(numAgents)+'_learn_system_phgnn_TrainLosses.pth')
    torch.save(TestsLosses, 'navigationTv'+str(numAgents)+'_learn_system_phgnn_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Navigation problem with time-varying communications, solved via the port-Hamiltonian GNN')
