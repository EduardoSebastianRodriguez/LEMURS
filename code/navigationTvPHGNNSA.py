import torch
import argparse
from navigationTvFunctions import learnNavigationGNNSA, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    learn_system = learnNavigationGNNSA(parameters)
    ...
    # Build dataset
    inputs   = torch.load('navigationTv'+str(numAgents)+'_inputsTrain_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth').to(device)
    target_2 = torch.load('navigationTv'+str(numAgents)+'_target2Train_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth').to(device)

    # Build evaluation
    inputs_eval   = torch.load('navigationTv'+str(numAgents)+'_inputsTests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth').to(device)
    target_eval_2 = torch.load('navigationTv'+str(numAgents)+'_target2Tests_'+str(numTrain)+str(numTests)+str(numSamples)+str(seed)+'_.pth').to(device)
    current       = 100000000.0
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs-1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system, 'navigationTv'+str(numAgents)+'_learn_system_phgnnsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'navigationTv'+str(numAgents)+'_learn_system_phgnnsa_TrainLosses.pth')
    torch.save(TestsLosses, 'navigationTv'+str(numAgents)+'_learn_system_phgnnsa_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Navigation problem with time-varying communications, solved via the port-Hamiltonian GNN-SA')
