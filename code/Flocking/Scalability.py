...
for i in range(len(agentsTrain)):
    for j in range(len(agentsTests)):
        losses[i, j, 0] = np.load('F_mean' + str(agentsTrain[i]) + str(agentsTests[j]) + '_loss_LEMURS.npy')
        losses[i, j, 1] = np.load('F_std' + str(agentsTrain[i]) + str(agentsTests[j]) + '_loss_LEMURS.npy')
