...
def main(numTrain=20000, numTests=20000, numSamples=5, seed_data=42, seed_train=42, numAgents=4):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'F'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phgnnsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'F'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phgnnsa_TrainLosses.pth')
    torch.save(TestsLosses, 'F'+str(numAgents)+'_'+str(seed_train)+'_learn_system_phgnnsa_TestsLosses.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GNNSA training for the flocking problem')
