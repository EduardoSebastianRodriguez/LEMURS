...
class learnSystem(nn.Module):
    ...
    def flocking_dynamics_assess(self, inputs):
        ...
        # Input for H
        state_h = torch.zeros((state.shape[0], 6 * self.na), device=self.device)
        state_h[:, 0::6] = torch.kron(stateD[:, :, 0], torch.ones(self.na, 1, device=self.device))
        state_h[:, 1::6] = torch.kron(stateD[:, :, 1], torch.ones(self.na, 1, device=self.device))
        state_h[:, 2::6] = torch.kron(stateD[:, :, 2], torch.ones(self.na, 1, device=self.device))
        state_h[:, 3::6] = torch.kron(stateD[:, :, 3], torch.ones(self.na, 1, device=self.device))
        state_h[:, 4::6] = ((pos.reshape(-1, self.na, 2)).norm(p=2, dim=2) + self.epsilon).pow(-1).unsqueeze(2).reshape(-1, self.na)
        state_h[:, 5::6] = (pos.reshape(-1, self.na, 2)).norm(p=2, dim=2).unsqueeze(2).reshape(-1, self.na)
        LLL               = torch.kron(LL, torch.ones((1, 1, 6), device=self.device))
        LLL[:, :, 4::6]   = Q
        ...
        # H
        inputsL = Variable(state_h.reshape(-1, self.na, 6).transpose(1, 2).data, requires_grad=True)
...
class learnSystemGNNSA(nn.Module):
    ...
    def flocking_dynamics(self, t, inputs):
        ...
        state_h = torch.zeros((inputs.shape[0], 4, self.na, self.na), device=self.device)
        state_h[:, 0, :, :] = torch.bmm(maks_l, state[:, 0, :, :])
        state_h[:, 1, :, :] = torch.bmm(maks_l, state[:, 1, :, :])
        state_h[:, 2, :, :] = torch.bmm(maks_l, state[:, 2, :, :])
        state_h[:, 3, :, :] = torch.bmm(maks_l, state[:, 3, :, :])
        state_h = state_h.transpose(1, 2).transpose(2, 3).reshape(-1, 4 * self.na)
        ...
        state_h = state_h.reshape(-1, self.na, self.na, 6)
        state_h = state_h.transpose(2, 3).transpose(1, 2)
        state_h[:, 0, :, :] = torch.bmm(maks_l, state_h[:, 0, :, :])
        state_h[:, 1, :, :] = torch.bmm(maks_l, state_h[:, 1, :, :])
        state_h[:, 2, :, :] = torch.bmm(maks_l, state_h[:, 2, :, :])
        state_h[:, 3, :, :] = torch.bmm(maks_l, state_h[:, 3, :, :])
        state_h[:, 4, :, :] = torch.bmm(maks_q, state_h[:, 4, :, :])
        state_h[:, 5, :, :] = torch.bmm(maks_l, state_h[:, 5, :, :])
        state_h = state_h.transpose(1, 2).transpose(2, 3).reshape(-1, 6 * self.na)
