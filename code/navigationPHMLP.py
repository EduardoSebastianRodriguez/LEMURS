import torch
import argparse
from navigationFunctions import learnNavigationMLP, L2_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system, 'navigation'+str(numAgents)+'_learn_system_phmlp.pth')
    ...
    # Save
    torch.save(TrainLosses, 'navigation'+str(numAgents)+'_learn_system_phmlp_TrainLosses.pth')
    torch.save(TestsLosses, 'navigation'+str(numAgents)+'_learn_system_phmlp_TestsLosses.pth')
