import torch
import argparse
from flockingFunctions import learnFlockingGNNSA, L2_loss, LD_loss, LH_loss, LV_loss
...
def main(numTrain=2000, numTests=400, numSamples=5, seed=42, numAgents=4):
    ...
    learn_system = learnFlockingGNNSA(parameters)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0 or epoch == epochs - 1:
            ...
            if current > loss.detach().cpu().numpy():
                torch.save(learn_system.state_dict(), 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phgnnsa.pth')
    ...
    # Save
    torch.save(TrainLosses, 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phgnnsa_TrainLosses.pth')
    torch.save(TestsLosses, 'flocking'+str(numAgents)+'_'+str(seed)+'_learn_system_phgnnsa_TestsLosses.pth')
